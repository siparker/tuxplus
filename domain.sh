...
EOF
	...
	cat > $DOMAIN_PATH/cleanup.sh <<EOF

	#!/bin/sh
	find $DOMAIN_PATH/public_html/var/session -name 'sess_*' -type f -mtime +7 -exec rm {} \;
	
EOF

chmod u+x $DOMAIN_PATH/cleanup.sh


crontab -l > /tmp/currentcron
echo "@daily $DOMAIN_PATH/cleanup.sh " >> /tmp/currentcron
...
server {
		...
		location ~ /(app|var|includes|pkginfo|errors/local.xml)/ {
			deny all;
		}
...
<VirtualHost *:80>
    ...
    FastCGIExternalServer $DOMAIN_PATH/php7.0-fpm -pass-header Authorization -idle-timeout 120 -socket /var/run/php/php7.0-fpm-$DOMAIN_OWNER.sock
    Alias /php7.0-fcgi $DOMAIN_PATH
...
if [ ! -n "$1" ]; then
    ...
    echo -n  "$0"
    echo -ne "\033[36m add user [magento|wordpress|laravel|normal] Domain.tld\033[0m"
